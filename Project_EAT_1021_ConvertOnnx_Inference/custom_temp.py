...
dataset_dir = r"E:\syz_data_2025_04_21\datasets\train"  # train_dataset 文件夹路径
...
class Custom22Dataset(TorchDataset):
    def __init__(self, dataset_dir, split="train", clip_length=5, resample_rate=32000, gain_augment=0):
        """
        ...
        # 遍历所有 fold
        all_samples = []
        for fold_name in os.listdir(dataset_dir):
...
# ---------------- Dataset 获取接口 -----------------
def get_custom22_train_set(resample_rate=32000, roll=False, wavmix=False, gain_augment=0):
    ds = Custom22Dataset(dataset_dir, split="train", resample_rate=resample_rate, gain_augment=gain_augment)
    if roll:
...
def get_custom22_valid_set(resample_rate=32000):
    ds = Custom22Dataset(dataset_dir, split="val", resample_rate=resample_rate, gain_augment=0)
    return ds
