...
def train(args):
    ...
    if samples_per_cls == None:
        ...
        # 假设 samples_per_cls 之前已经准备好（你的代码中已有）
    ...
    for epoch in range(args.n_epochs):
        ...
        train_stats = {"total_loss": [], "cb_loss": [], "supcon_loss": [], "ce_loss": []}
        pbar = tqdm(dl, desc=f"Epoch {epoch+1}/{args.n_epochs}")
        for batch in pbar:
            x, f, y = batch                  # x: waveform batch, y: labels (one-hot or int?)
            bs = x.size(0)
            x = x.to(device)
            ...
            y_hat, embedding = model(x_mel)  # y_hat: [B, num_classes], embedding: [B, feat_dim] 或 [B, N, feat_dim]

            # --- 1) CBLoss: 需要 logits 与 labels（你的 CBLoss 接口之前使用过： CBLoss(logits, labels)）
            # 如果 CBLoss 需要 one-hot 或整数，按照你的实现传参；这里假设它接收 logits 和整数标签
            cb_loss_val = CBLoss(y_hat, y)   # scalar tensor
            ...
            # --- 合并 loss
            loss = cb_w * cb_loss_val + sup_w * supcon_loss_val + ce_w * ce_loss_val
...
if __name__ == '__main__':
    ...
    parser.add_argument('--no_roll', action='store_true', default=True)
    parser.add_argument('--no_wavmix', action='store_true', default=True)
