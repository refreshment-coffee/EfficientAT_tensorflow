...
load_pt_path=None,
    **kwargs: Any,
    ...
    if load_pt_path:
        state_dict = torch.load(load_pt_path)
        print("load : {}",load_pt_path)
        try:
...
def get_model(num_classes: int = 527, pretrained_name: str = None, width_mult: float = 1.0,
              ...
              input_dim_t: int = 1000, se_dims: str = 'c', se_agg: str = "max", se_r: int = 4,load_pt_path=None):
    """
    ...
    m = mobilenet_v3(pretrained_name=pretrained_name, num_classes=num_classes,
                     ...
                     input_dims=input_dims, se_conf=se_conf,load_pt_path=load_pt_path
                     )
    print(m)
    return m
