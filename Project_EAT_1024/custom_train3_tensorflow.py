...
def train(args):
    ...
    # 训练 DataLoader
    dl = DataLoader(dataset=get_custom22_train_set(dataset_dir=dataset_dir,resample_rate=args.resample_rate,
    ...
    # 验证 DataLoader
    eval_dl = DataLoader(dataset=get_custom22_valid_set(dataset_dir=dataset_dir,resample_rate=args.resample_rate,class2idx=CLASS2IDX),
                         worker_init_fn=worker_init_fn,
                         num_workers=args.num_workers,
                         batch_size=args.batch_size)
...
#             param.requires_grad = True
    ...
    if samples_per_cls == None:
        samples_per_cls_temp_dataset =DataLoader(dataset=get_custom22_train_set(dataset_dir=dataset_dir,resample_rate=args.resample_rate,
                                                   roll=False if args.no_roll else True,
                                                   wavmix=False if args.no_wavmix else True,
                                                   gain_augment=args.gain_augment,class2idx=CLASS2IDX),
                    worker_init_fn=worker_init_fn,
