...
from customDataset import get_custom22_valid_set, get_custom22_train_set,Custom22Dataset
...
CUSTOM_CLASSES =['000Rain','001Wind','002Thunder',
              ...
              '009Person','010Other','0111Drill_Chaninsaw',
              '012Knock','013Aerodynamic','01144Mech_work_Engine',
...
CLIP_LENGTH=2 ##训练的时候，选择多少音频长度---duration -----------修改后记得修改batch_size==== 2---32    4---16
##影响Mel后的特征通道处， 间接影响后续onnx导出时的input_shape =====###  4s---32,128,400    2s---32,128,200
## 1022 注意，SYZ构造的音频长度均为2s

DATE=f"1022_LEN_{CLIP_LENGTH}_"
import wandb
...
def compute_samples_per_cls(dataset):
    ...
    for _,_, label in tqdm(dataset):
        if torch.is_tensor(label):
            label = label
        for tt in list(label):
            label_counter[int(tt)] += 1
...
def train(args):
    ...
    # 训练 DataLoader
    dl = DataLoader(dataset=get_custom22_train_set(dataset_dir=dataset_dir,clip_length=CLIP_LENGTH,resample_rate=args.resample_rate,
                                                   roll=False if args.no_roll else True,
                                                   wavmix=False if args.no_wavmix else True,
                                                   gain_augment=args.gain_augment,class2idx=CLASS2IDX),
    ...
    # 验证 DataLoader
    eval_dl = DataLoader(dataset=get_custom22_valid_set(dataset_dir=dataset_dir,clip_length=CLIP_LENGTH,resample_rate=args.resample_rate,class2idx=CLASS2IDX),
                         worker_init_fn=worker_init_fn,
                         num_workers=args.num_workers,
                         batch_size=args.batch_size)
    ...
    ##原始label情况
    samples_per_cls=[13062, 8153, 7422, 13569, 5702, 10557, 4572, 3450, 8384, 6772, 5310, 6418, 3875, 2881, 4093, 4570, 2666, 3152, 2715, 8343]
    if samples_per_cls == None:
        print("samples_per_cls is None ! ")
        samples_per_cls_temp_dataset =DataLoader(dataset=get_custom22_train_set(dataset_dir=dataset_dir,clip_length=CLIP_LENGTH,resample_rate=args.resample_rate,
                                                   roll=False if args.no_roll else True,
                                                   wavmix=False if args.no_wavmix else True,
                                                   gain_augment=args.gain_augment,class2idx=CLASS2IDX),
                    worker_init_fn=worker_init_fn,
    ...
    for epoch in range(args.n_epochs):
        ...
        for batch in pbar:
            ...
            # forward: mel + model
            x_mel = _mel_forward(x, mel)    
            # print(np.array(x_mel.cpu()).shape)
            # exit()
        ...
        name = f"{model_name}_{DATE}_custom22_epoch_{epoch}_acc_{int(round(accuracy * 1000))}.pt"
...
def _mel_forward(x, mel): 
    """
    x--- #(128, 1, 64000)  pytorch数据

    mel，此py中为tensorflow实现版本，仅支持numpy输入
    """

    """
    以下是输出mel后的特征shape， 为了后续onnx导出时知道设置多少input_shape

    ###  4s---32,128,400    2s---32,128,200
    """
        
    # print(x.shape)  #(128, 1, 64000)
    # old_shape = x.size()
    ...
    # x= torch.tensor(x, dtype=torch.float32).contiguous().to(DEVICE)
    ##torch.Size([128, 64000])   ##bs,64000
    ...
    waveform=x
    waveform_np = waveform.detach().cpu().numpy().reshape(-1, waveform.shape[-1])
    mel_spec = mel(waveform_np)
    mel_spec = np.array(mel_spec, dtype=np.float32)

    if INITIAL_PRINT:
        print("after_mel : x.shape : ",mel_spec.shape)
        INITIAL_PRINT=False
    mel_spec = torch.tensor(mel_spec, dtype=torch.float32).contiguous().to(DEVICE)
    mel_spec = mel_spec.view(waveform.shape[0], 1, mel_spec.shape[1], mel_spec.shape[2])
    # exit()
    return mel_spec
